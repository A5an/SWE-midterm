...
| FR-UM-05 | Unauthorized action handling | User attempts forbidden action from UI or API. | System returns `403 Forbidden` with error code `AUTHZ_FORBIDDEN`, performs no state mutation, and logs the denied attempt with request ID. | In role-matrix tests, every forbidden operation returns `403` + `AUTHZ_FORBIDDEN`; document hash and version count remain unchanged after each denied attempt. |
