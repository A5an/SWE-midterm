...
- Evidence generated from commit: `d3d4209cf3f6f9e76797de8346966e7d37d0b91c`
- PR: `https://github.com/A5an/SWE-midterm/pull/7` (also recorded in `docs/evidence/LIN-TEA-10/pr-link.txt`). This evidence is intentionally anchored to the evidence-producing commit instead of the moving branch head.
...
- Revalidated current document permissions on websocket upgrade and collaboration message handling, so downgraded or removed users cannot continue mutating through an already-issued session token.
...
- `role-matrix-tests.txt`: full `npm test` log covering unshared-user denial, username/email share grants, owner/editor/viewer role matrix, viewer direct-edit denial, viewer session denial, collaboration reconnect behavior, and stale-session revocation after share downgrade.
...
- `pr-link.txt`: direct PR URL for this branch.
