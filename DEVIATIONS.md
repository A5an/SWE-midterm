...
|---|---|---|---|---|---|---|---|---|---|
| D-001 | planned | A1 architecture specifies API container as `NestJS + TypeScript` | A2 implementation constraint is `FastAPI + Python`, but current backend on this branch is still Node.js PoC | Hard technology constraint in Assignment 2 | Improvement (required compliance change) | FastAPI migration must preserve contract behavior while introducing JWT/auth flows | Dachi | 2026-04-15 | `docs/architecture/Part2_System_Architecture.md:64`; `apps/api/src/server.ts:1`; `README.md:11-14` |
