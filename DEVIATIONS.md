...
| D-010 | implemented | A1/A2 baseline expects user-visible version history/restore as part of the full document lifecycle | TEA-11 delivers immutable version list/fetch/restore APIs plus collaboration reload events in `apps/api`, but a dedicated frontend version-history UI is still deferred | The baseline needed defensible server-side version semantics and restore safety first, without widening scope into a new frontend workflow during the same issue | Compromise (scope sequencing) | Core version history and revert-as-new-head behavior are now testable through API contracts and active collaboration sessions; discoverable UI for browsing/restoring versions remains follow-up work | Dachi | 2026-04-19 | `apps/api/src/server.ts:1`; `apps/api/src/contract-test.ts:1`; `packages/contracts/src/index.ts:1`; `apps/web/src/App.ts:1`; `docs/evidence/LIN-TEA-11/README.md:1` |
